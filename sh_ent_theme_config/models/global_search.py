...
class GlobalSearch(models.Model):
    ...
    main_field_id = fields.Many2one('ir.model.fields',string="Name Field",required=True,domain="[('model_id','=',model_id)]")
                ...
                try:
                    ...
                    if company_id_field:
                        if single_company ==1:
                            ...
                            if count == 1:
                                model_obj = self.env[search_rec.model_id.model].search_read(['|',('company_id','=',False),('company_id','=',company.id)],field_list,order='id')
                                count = 0
                            else:
...
class GlobalSearchFields(models.Model):
    ...
    def sh_o2m_dynamic_action_action(self):
        if self.ttype == 'one2many':
            view = self.env.ref('sh_ent_theme_config.sh_o2m_global_search_form')
            return {
            ...
            view = self.env.ref('sh_ent_theme_config.sh_m2o_global_search_form')
            return {
