        ...
        if self.user_ids:
            if self.simple_text:
                if self.notification_type == 'danger':
                    CODE_SOUND_SUCCESS = "SH_NOTIFICATION_FAIL_" 
                    for user in self.user_ids:
                        notifications.append([
                            (self._cr.dbname, 'res.partner', user.partner_id.id),
                            {'type': 'simple_notification', 'title': _('Alert'), 'message': CODE_SOUND_SUCCESS + self.description_text, 'sticky': True, 'warning': True}  # sorted to make deterministic for tests
                        ])
                if self.notification_type == 'warning':
                    CODE_SOUND_FAIL = "SH_NOTIFICATION_SUCCESS_" 
                    for user in self.user_ids:
                        notifications.append([
                            (self._cr.dbname, 'res.partner', user.partner_id.id),
